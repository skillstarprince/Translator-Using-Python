...
def main_exit():
    rr=messagebox.askokcancel('Notification','Are you want to exit')
    if(rr == True):
        root.destroy()
    else:
        pass
...
font_box.current(37)
font_box.place(x=380,y=0)

#################################Entry Box######################
varname1 = StringVar()
entry1 = Entry(root,width=30,textvariable=varname1,font = ('times',14,'italic bold'))
entry1.place(x=150,y=40)

varname2 = StringVar()
entry2 = Entry(root,width=30,textvariable=varname2,font = ('times',14,'italic bold'))
entry2.place(x=150,y=100)

###############################label############################

label1 = Label(root,text='Enter Words:',font = ('times',14,'italic bold'),bg='pink')
label1.place(x=5,y=40)

label2 = Label(root,text='Translated:',font = ('times',14,'italic bold'),bg='pink')
label2.place(x=5,y=100)

label3 = Label(root,text='',font = ('times',14,'italic bold'),bg='pink')
...
btn2 = Button(root,text='Exit',bd=10,bg='yellow',activebackground='red',width=100,font = ('times',14,'italic bold'),
              image = imgbt2,compound =RIGHT,command=main_exit)
btn2.place(x=280,y=170)

root.bind('<Return>',tt)   #Return is used for 'enter' key
